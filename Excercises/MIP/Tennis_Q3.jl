...
for p in 1:P
for pp in 1:P
Distance[p,pp] = sqrt( (x_pos[p]-x_pos[pp])*(x_pos[p]-x_pos[pp])
+ (y_pos[p]-y_pos[pp])*(y_pos[p]-y_pos[pp]) )
end
end
...
@constraint(tennis, [p=1:P],sum(x[pp,p] for pp=1:P) + s[p] == sum(x[p,pp] for pp=1:P) + e[p])
# one start
...
if termination_status(tennis) == MOI.OPTIMAL
    ...
    for p=1:P
        if value(s[p])>0.1
            println("Start point: $(Points[p])")
        end
        if value(e[p])>0.1
            println("End point: $(Points[p])")
        end
    end
else
    println(" No solution")
...
#Print x[p, pp] from starting point to end point


for p in 1:P, pp in 1:P
    if value(x[p,pp]) > 0.5
        println("$(Points[p]) -> $(Points[pp])")
    end
end
println("--------------------------------------");
#************************************************************************
